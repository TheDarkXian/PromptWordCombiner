...
- 支持 JSON 全量备份、恢复和合并。
- 支持单独导入和导出模板 JSON；其他 AI 可以按照 `docs/TEMPLATE_JSON_GUIDE.md` 生成可导入模板。
...
当前同时支持手动提示词工作流和可选的 AI 文本生成步骤；模板可以决定生成结果写入步骤输出或模板变量。
...
- `inputs`：全局输入变量。
- `steps`：有顺序的提示词步骤。
- `groupId`：模板所属的用户自建模板分组；为空时属于“未分组模板”。
- `createdAt`、`lastModifiedAt`：模板创建与最近修改时间；旧模板加载时自动补齐。
- `hideProjects`：控制该模板下的项目是否参与“全部项目”和“最近修改”总览；只影响显示，不删除项目。
...
| `Sidebar.tsx` | 左侧栏：文件库导航与模板分组，或项目内的变量、步骤导航、变量提取和烘焙导出。 |
| `FileLibrary.tsx` | 文件库主页：项目视图、模板管理、模板 JSON 导入导出、统一新建项目入口、排序、按模板批量打开项目、备份、恢复和合并入口。 |
...
| `services/templateJsonService.ts` | 单独模板 JSON 的校验、规范化导入和交换格式导出。 |
...
左侧栏和项目右侧提示词全览面板在拖动调整宽度时，只通过 CSS 变量更新实时宽度；鼠标松开后才提交 React 状态并持久化，避免每个鼠标移动事件重新渲染完整项目流程。

文件库采用“侧边栏导航 + 主内容区”结构。侧边栏提供全部项目、最近修改、全部模板、未分组模板、用户模板分组和具体模板；主区域只显示当前导航项对应的项目或模板管理表。模板分组保存在 `AppSettings.templateGroups`，通过 `parentId` 支持组嵌套组；模板通过 `groupId` 归属分组。删除分组时直属模板和子分组上移一级，不删除模板或项目。

新建项目使用文件库顶部的统一入口：选中具体模板时直接使用该模板；其他项目视图先打开模板选择器。模板管理表负责编辑、复制和删除模板。模板移动分组、模板顺序、分组嵌套和同级分组顺序统一通过文件库侧边栏拖放管理，不提供分组下拉框。

模板分组整行都可以点击展开或折叠，不要求精确点击小箭头；行内“查看”按钮用于进入该分组及子分组的项目视图。模板和分组不使用独立拖动把手，长按整行约 350 毫秒即可进入拖动。模板分组也支持全部折叠或全部展开，折叠状态会持久化。拖动时界面显示跟随鼠标的对象名称以及当前有效放置目标。

模板分组通过右键菜单创建：右键模板目录空白区域可以新建根分组，右键某个分组可以新建子分组、重命名或删除。创建分组名称使用项目内弹窗输入，不在目录树中插入临时输入框，也不使用浏览器原生弹窗。

模板管理表支持按名称、创建时间、最近修改、项目数量和步骤数量排序；表头列之间可以拖动调整宽度。模板排序和列宽属于本地界面设置，关闭应用后会继续保留。顶部项目标签支持右键“关闭其他标签”和“关闭全部项目标签”。

模板管理表保存的是用户偏好的列宽；当主区域宽度不足时，各列会在最小宽度范围内自动收缩，操作按钮允许换行，避免表格依赖横向滚动条。

模板管理表最前面的眼睛列控制项目是否参与“全部项目”和“最近修改”总览。关闭后，仍可通过侧边栏具体模板或模板分组查看其项目；已打开标签、项目数据、模板和备份内容不受影响。全部模板页面提供一键显示所有模板项目和一键隐藏所有模板项目。

“全部项目”会将当前眼睛开启的项目按模板分组展示。每个模板项目组可以折叠、创建项目、打开此模板的所有项目或进入该模板项目视图；全部折叠、全部展开和各组折叠状态会持久化。找不到关联模板的孤立项目显示在“模板已失效”组中。
...
项目禁止使用浏览器原生 `window.alert`、`window.confirm` 和 `window.prompt`。确认、提醒和文本输入必须使用项目内模态框、内联编辑、日志面板或 Toast。
...
- 模板步骤可以通过 `showInPromptOverview` 控制是否出现在项目右侧“拼接结果实时全览”中；旧模板默认显示。关闭只影响右侧全览，不影响步骤卡片、复制、AI 生成和步骤引用。
- 项目步骤折叠状态在当前应用会话内由所有项目共用；切换项目、文件库或其他页面后保留，重启应用后清空。
- 步骤提示词复制成功后，复制按钮会在当前应用会话内显示为“已复制”，用于标记操作进度；切换项目或页面后保留，重启应用后清空，不写入项目数据或备份 JSON。已复制按钮仍可再次点击复制。
- 关闭当前项目标签时优先打开右侧紧邻的下一个项目；关闭最后一个项目标签时回到左侧上一个，避免跳到无关项目打乱连续工作顺序。
...
模板编辑器左侧分为“变量”“模型变量”“提取预设”三个标签。模板提取预设保存在 `template.extractionPresets`，负责指定原始文本变量、目标变量、AI 补全模型变量和可选整体说明。项目变量如果被设置为某个预设的原始文本，会在变量旁显示“提取”入口；点击后直接打开变量提取标签中的对应预设。一个原始变量绑定多个预设时先选择预设。

变量提取工具同时支持模板预设模式与临时提取模式。预设模式锁定原始文本、目标变量和模型配置，但仍要求用户检查结果并确认写入；临时模式保留手动输入和选择能力。删除模板变量或模型变量时，应同步清理提取预设中的失效绑定。
...
变量提取的原始文本、目标变量选择和结果确认状态按“项目 ID + 提取预设 ID（或临时模式）”缓存在当前应用会话内，用于切换标签后恢复草稿；这些草稿不写入项目数据，也不会进入导出的备份 JSON。模板提取预设本身属于模板数据，会进入备份 JSON。
